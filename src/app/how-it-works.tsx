...
export default function HowItWorks(){const {language}=useAppLanguage();if(language==="zh")return <ChineseGuide/>;return <ScrollView style={s.page} contentContainerStyle={s.content}>
  ...
  <Section title="Our purpose and supported tickets">
    <P>Formal Exchange exists first to prevent valid Oxford, Cambridge and Durham formal dinner places from being wasted when the original holder can no longer attend. Formal trading remains the platform’s primary service and is the default view on the buyer page.</P>
    ...
    <P>As a secondary service, members may also list coach tickets, train tickets, other event admissions and airport ride-shares. Every listing is marked with its university. Journey listings state origin, destination, departure, arrival, automatically calculated duration and relevant restrictions. Event posts use only the event name, places, time and description; ride-shares use pickup, destination, time, places and fare-sharing details.</P>
  ...
  <Section title="Who may use the platform">
    ...
    <P>We strongly recommend that Oxford, Cambridge and Durham students verify their accounts. The verified badge increases buyer trust and is required for Formal ticket publishing. Our verification scope will expand to enterprise and global educational emails in the future.</P>
  ...
  <Section title="College visit records">
    ...
    <P>The college saved in a member’s profile is counted as visited automatically. Other visits may be recorded manually or through completed activity after the event date; arranging a ticket before the event does not itself prove attendance.</P>
  </Section>
...
function ChineseGuide(){return <ScrollView style={s.page} contentContainerStyle={s.content}>
  ...
  <Section title="平台目的与支持的票务"><P>我们的首要目标，是在原持票人无法参加时，避免有效的牛津、剑桥和杜伦 Formal 晚宴名额被浪费。Formal 交换始终是主营功能，也是买家页的默认内容。</P><P>Formal 经常一票难求，但抢到票的人可能临时无法参加。部分学院不提供退款或官方转售渠道，跨学院及牛津—剑桥之间的机会又分散在多个非正式渠道。平台只撮合学院规则允许的供需，不会凌驾于学院规则之上。</P><P>辅助功能包括大巴票、火车票、其他活动门票和机场拼车。所有帖子标明牛津或剑桥；行程票展示出发地、目的地、出发与到达时间、自动计算的时长及限制。</P></Section>
