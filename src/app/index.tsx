...
export function MarketplaceHome() {
  ...
  const [filterUni, setFilterUni] = useState<"all" | University>("all");
  const [filterDate, setFilterDate] = useState("");
  const [filterOrigin, setFilterOrigin] = useState("");
  const [filterDest, setFilterDest] = useState("");
  const [applied, setApplied] = useState<{kw:string;uni:"all"|University;date:string;origin:string;dest:string}>({kw:"",uni:"all",date:"",origin:"",dest:""});
  ...
  return (
    <View style={{ flex: 1 }}>
      <ScrollView keyboardShouldPersistTaps="handled" style={s.page} contentContainerStyle={[s.c, mobile && s.cMobile]}>
        <View style={s.top}>
          <View>
            <Text style={[s.brand, mobile && s.brandMobile]}>FORMAL EXCHANGE</Text>
            <Text style={s.sub}>{text("Formal tickets first · Oxford, Cambridge & Durham", "主营 Formal 票交换 · 牛津、剑桥与杜伦")}</Text>
          </View>
        ...
        {mobile ? <>
          <View style={s.mobileCategoryRow}>
            <ScrollView keyboardShouldPersistTaps="handled" horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.mobileCategoryScroll}>
        ...
        {filtersOpen ? <View style={s.filterBar}>
          ...
          {showUni ? <View style={s.filterRow}>
            ...
            <View style={s.filterChips}>
              {(["all","Oxford","Cambridge","Durham"] as const).map(u => <Pressable key={u} style={[s.filterChip, filterUni === u && s.filterChipOn]} onPress={() => setFilterUni(u)}><Text style={[s.filterChipText, filterUni === u && s.filterChipTextOn]}>{u === "all" ? text("All","全部") : u}</Text></Pressable>)}
            </View>
          </View> : null}
...
function WebLanding(){const {language,text}=useAppLanguage();const {width}=useWindowDimensions();const compact=width<760;return <ScrollView keyboardShouldPersistTaps="handled" style={s.landing} contentContainerStyle={s.landingContent}>
  ...
  <View style={[s.heroLanding,compact&&s.heroLandingCompact]}><View style={[s.heroCopy,compact?s.heroCopyCompact:s.heroCopyRow]}><Text style={s.landingEyebrow}>{text("OXFORD · CAMBRIDGE · DURHAM · COMMUNITY MARKETPLACE","牛津 · 剑桥 · 杜伦 · 校园社区平台")}</Text><Text style={[s.landingTitle,compact&&s.landingTitleCompact]}>{text("Help a hard-won Formal place find someone who can use it.","让来之不易的 Formal 名额，不再被白白浪费。")}</Text><Text style={s.landingLead}>{text("Formal tickets are scarce, yet plans change and official refund or resale routes are often limited. Formal Exchange brings permitted listings, buyer requests, travel tickets, ride-shares and college information into one clear place, across Oxford, Cambridge and Durham.","Formal 一票难求，但持票人也可能临时无法参加，而官方退款或转售渠道往往有限。Formal Exchange 将牛津、剑桥与杜伦的合规帖子、买家需求、车票拼车与学院信息集中在一个清晰的平台中。")}</Text><Pressable style={s.landingSwapHero} onPress={()=>router.push("/marketplace?filter=swap")}><View style={s.landingSwapHeroIcon}><Ionicons name="swap-horizontal" size={23} color="#fff"/></View><View style={{flex:1}}><Text style={s.landingSwapHeroTitle}>{text("Ticket Swap is a core part of Formal Exchange","换票专区是 Formal Exchange 的核心功能")}</Text><Text style={s.landingSwapHeroText}>{text("Exchange one college ticket for another without requiring a cash sale. Any price difference remains your private decision.","不必先买卖，就能用一个学院的票直接交换另一个学院的票；是否补差价由双方自行决定。")}</Text></View><Ionicons name="arrow-forward" size={20} color="#9A3412"/></Pressable><View style={[s.landingActions,compact&&s.landingActionsCompact]}><Pressable style={s.landingPrimary} onPress={()=>router.push("/marketplace")}><Text style={s.landingPrimaryText}>{text("Enter web app","进入网页版")}</Text><Ionicons name="arrow-forward" size={19} color="#fff"/></Pressable><Pressable style={s.landingSecondary} onPress={()=>openAppStore(text)}><Ionicons name="phone-portrait-outline" size={19} color="#071B3A"/><Text style={s.landingSecondaryText}>{text("Download app","下载 APP")}</Text></Pressable></View></View><View style={[s.heroPanel,compact?s.heroPanelCompact:s.heroPanelRow]}><View style={s.heroPanelIcon}><Ionicons name="restaurant-outline" size={34} color="#fff"/></View><Text style={s.heroPanelTitle}>{text("Formal first","主营 Formal")}</Text><Text style={s.heroPanelText}>{text("See eligibility, entry rules, dates, prices and transfer limits before making contact.","联系前即可查看资格、入场规则、日期、价格和转让限制。")}</Text><View style={s.heroSteps}>{[["1",text("Browse or request","浏览或求票")],["2",text("Check the rules","核对规则")],["3",text("Message safely","安全私信")]].map(([n,label])=><View key={n} style={s.heroStep}><Text style={s.heroStepNo}>{n}</Text><Text style={s.heroStepText}>{label}</Text></View>)}</View></View></View>
  <View style={s.landingGrid}>{[["shield-checkmark-outline",text("Verified community","认证社区"),text("Register with any email; users who verify with a UK .ac.uk academic email receive a verified badge. Formal tickets can be published from verified Oxford, Cambridge and Durham accounts — keeping the marketplace safe and trustworthy. More collegiate universities will follow.","任意邮箱即可注册；使用英国 .ac.uk 高校邮箱认证后将获得认证标识。牛津、剑桥和杜伦的认证账号可以发布 Formal 票，保障平台安全可信。今后将开放更多书院制大学。")],["swap-horizontal-outline",text("Ticket swaps","换票专区"),text("Find members open to exchanging one ticket for another, with any price difference agreed privately.","寻找愿意票换票的用户；是否补差价由双方私下商定。")],["language-outline",text("Chinese or English","中英文自动适配"),text("Write once in your app language. Optional text is translated for readers using the other language, with the original available.","只需使用当前界面语言填写一次；选填文字自动翻译给另一语言用户，并保留原文。")]].map(([icon,title,body])=><View key={String(title)} style={s.landingFeature}><Ionicons name={icon as any} size={27} color="#9A3412"/><Text style={s.landingFeatureTitle}>{title}</Text><Text style={s.landingFeatureText}>{body}</Text></View>)}</View>
  <View style={s.landingFooter}><Text style={s.landingFooterBrand}>FORMAL EXCHANGE</Text><Text style={s.landingFooterText}>{text("Support: support@formal-exchange.co.uk","支持邮箱：support@formal-exchange.co.uk")}</Text><View style={s.landingFooterLinks}><Pressable onPress={()=>router.push("/privacy-policy")}><Text style={s.landingFooterLink}>{text("Privacy Policy","隐私政策")}</Text></Pressable><Text style={s.landingFooterDot}>·</Text><Pressable onPress={()=>router.push("/terms-and-conditions")}><Text style={s.landingFooterLink}>{text("Terms & Conditions","条款与条件")}</Text></Pressable></View></View>
</ScrollView>}
...
const HELP_LINKS: { keys: string[]; en: string; zh: string; route: string; icon: string }[] = [
  ...
  { keys: ["terms", "condition", "条款", "条件"], en: "Terms & conditions", zh: "条款", route: "/terms-and-conditions", icon: "document-text-outline" },
  { keys: ["privacy", "data", "gdpr", "policy", "隐私", "数据", "政策"], en: "Privacy policy", zh: "隐私政策", route: "/privacy-policy", icon: "lock-closed-outline" },
